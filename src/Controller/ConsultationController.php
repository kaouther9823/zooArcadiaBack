...
class ConsultationController extends AbstractController
{
    ...
    public function __construct( LoggerInterface $logger, SerializerInterface $serializer,  DocumentManager $documentManager)
    {
        $this->logger = $logger;
    ...
    public function consultAnimal(Request $request): JsonResponse
    {
        ...
        $consultation = $this->documentManager->getRepository(Consultation::class)->findOneBy(['animalName' => $animalName]);
        $logger = $this->logger;
        ...
        if (!$consultation) {
            ...
            $consultation->setAnimalHabitat($animalName);
            $consultation->setAnimalRace($animalName);
    ...
    {
        $consultations = $this->documentManager->getRepository(Consultation::class)->findAll();
        $logger = $this->logger;
